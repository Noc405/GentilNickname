...
class Database {
    ...
    public function getAllTeachers(){
        ...
        $queryRequest = "SELECT idTeacher, teaFirstname, teaName, teaNickname, teaVoix FROM `t_teacher` ORDER BY teaVoix DESC";
    ...
    public function getOneTeacher($id){
        // Recover the id, the firstname, the name and the nickname of all the teachers 
        $queryRequest = "SELECT idTeacher, teaFirstname, teaName, teaGender, fkSection, teaNickname, teaOrigine, teaVoix, t_section.secName FROM `t_teacher` 
     ...
     * Elect a teacher
     */
    public function electTeacher($id){
        //Get the number of the teacher has been elected
        $queryGetNumberElected = "SELECT teaVoix FROM `t_teacher` WHERE idTeacher = :idTeacher";
        //Set the array with binds
        $arrayWithBindsGetVoix = array(
            array("varName" => "idTeacher", "value" => $id, "type" => PDO::PARAM_INT)
        );
        //Do the query prepare request
        $numberElected = $this->queryPrepareExecute($queryGetNumberElected, $arrayWithBindsGetVoix);
        //Increment by one for enter the new value
        $newNumberElected = $numberElected[0]['teaVoix'] + 1;

        // Set the query to update the teacher
        $electQuery = "UPDATE `t_teacher` SET teaVoix = :voix WHERE idTeacher = :idTeacher;";
        //Create the array for the prepare query
        $arrayWithBinds = array(
            0 => array("varName" => "idTeacher", "value" => $id, "type" => PDO::PARAM_INT),
            1 => array("varName" => "voix", "value" => $newNumberElected, "type" => PDO::PARAM_INT)
        );
        //Do the query prepare
        $this->queryPrepareExecute($electQuery, $arrayWithBinds);
    }
 }
?>